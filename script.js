//Bài tập về nhà
// Bai 1: Khai báo 1 function
const firstExercise = () => {
    let firstNum = parseInt(prompt("Enter your first number"));
    let secondNum = parseInt(prompt("Enter your second number"));
    let thirdNum = parseInt(prompt("Enter your third number"));

    let result = calculate(firstNum, secondNum, thirdNum);
    alert(result);
}

function calculate(a, b, c) {
    if (a === 0) {
        alert("a không thể bằng 0");
    }
...
// Bai 2: kiểm tra tam giac
const secondExercise = () => {
    let a = parseInt(prompt("Nhap vao canh 1"));
    let b = parseInt(prompt("Nhap vap canh 2"));
    let c = parseInt(prompt("Nhap vap canh 3"));
    alert(isValidTriangle(a, b, c))
}
...
// Bai 3: --> Logic đơn gian, skip lam bài 4

// Bai 4: Thao tac tren model
let users = [
    {
        id: 1,
        first_name: "Kenton",
...
const forthExercise = () => {
    let idToFind = parseInt(prompt("Nhap user id can tim: "));
    ...
    let keywordToFind = prompt("Nhap vao chuoi ky tu can tim: ")
    for (let i = 0; i < users.length; i++) {
        if (users[i].first_name.includes(keywordToFind) || users[i].last_name.includes(keywordToFind)) {
            alert(users[i].email)
        }
    }
}